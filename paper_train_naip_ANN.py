...
train_folder = "data/training_data/4classes/naip/train_naip"  # Assumes you have downloaded our data and placed it at the repo root
test_folder = "data/training_data/4classes/naip/test_naip"
...
print(mean)
print(std)
print("NAIP with 0.6 resolution with ANN")
...
timestamp = time.time()
checkpoint_path = os.path.join(BASE_MODEL_OUTPUT_FOLDER, f"model-NAIP{model_name}-{timestamp}.pt")
bestmodel_path = os.path.join(BASE_MODEL_OUTPUT_FOLDER, f"best-test-model-NAIP{model_name}-{timestamp}.pt")
...
net = MortalityANN(input_size=[INPUT_CHANNELS] + INPUT_SIZE)
print(net)
