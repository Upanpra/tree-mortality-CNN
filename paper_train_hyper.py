...
print(mean)
print(std)
print("Hyperspectral with 426 bands at 1 m resolution with CNN RGB NIR 89")
...
channel_indices = [57, 31, 15]  # Set to None to not select any channels
if channel_indices is not None:
    INPUT_CHANNELS = len(channel_indices)

timestamp = time.time()
checkpoint_path = os.path.join(BASE_MODEL_OUTPUT_FOLDER, f"model-HYPER-RGB{model_name}-{timestamp}.pt")
bestmodel_path = os.path.join(BASE_MODEL_OUTPUT_FOLDER, f"best-test-model-HYPER-RGB{model_name}-{timestamp}.pt")
